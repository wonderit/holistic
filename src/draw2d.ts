...
export function draw2D(results: h.Results): void {
  ...
  // Pose...
  if (options.showSkeleton && options.renderBones) {
    drawingUtils.drawConnectors(ctx, results.poseLandmarks, h.POSE_CONNECTIONS, { color: 'white' });
  }
  if (options.showSkeleton && options.renderJoints) {
  ...
  // Hands...
  if (options.showSkeleton && options.renderHands) {
  ...
  // Face...
  if (options.showSkeleton && options.renderFace) {
